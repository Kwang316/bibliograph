...
if sorted_books:
    ...
    sorted_chapters = sorted(verses_by_book_chapter[default_book].keys(), key=lambda x: int(x) if x.isdigit() else 999)
else:
    default_book = None
    sorted_chapters = []
...
with col1:
    ...
    if selected_book:
        chapters = sorted(verses_by_book_chapter[selected_book].keys(), key=lambda x: int(x) if x.isdigit() else 999)
        with col_chapter:
    ...
    if selected_book and selected_chapter:
        verses = sorted(verses_by_book_chapter[selected_book][selected_chapter], 
                       key=lambda x: int(x.get('verse', 0)))
